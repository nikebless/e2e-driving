...
'''Visualize EBM energy values over time as a PyVista 3D surface plot on top of the drive RGB frames.

1. Easiest to run locally on a machine with a display.
2. Assumes the EBM energy values are stored in a numpy array with shape (n_frames, n_steering_samples)
3. Assumes the steering value samples are stored in a numpy array with shape (n_steering_samples, 1)
'''
